...
def grad_descent(theta, theta_prev = theta_prev):
    ...
    while np.linalg.norm(theta - theta_prev) > tolerance:
        ...
        opt_grads += [gradient]
        iter += 1
    return thetas, J_grad

theta_origin, J_grad_origin = grad_descent(theta)
theta_4, J_grad_4 = grad_descent(np.array([4,4]))
...
plot(theta_origin, J_grad_origin, 'g')
plot(theta_4, J_grad_4, 'r')
# Display the plot
plt.xlabel('theta0')
plt.ylabel('theta1')
